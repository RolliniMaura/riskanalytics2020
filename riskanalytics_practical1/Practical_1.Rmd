...
library(fExtremes)
blockMaxima(data$Average.Wait.Second, block = as.numeric(data$Date), doplot = TRUE)
findThreshold(data$Average.Wait.Second, n = floor(0.05*length(as.vector(data$Average.Wait.Second))), doplot = TRUE)

```
The total waiting time never exceeds 10'635 seconds in any given day.


```{method found online not sure what it is}
...
wait_xts <- xts(data$Average.Wait.Seconds, order.by = as.Date(data$Date))
ams <- apply.yearly(wait_xts, max)

# maximum-likelihood fitting of the GEV distribution
fit_mle <- fevd(as.vector(wait_xts[,1]), method = "MLE", type="GEV")
# diagnostic plots
plot(fit_mle)
# return levels:
