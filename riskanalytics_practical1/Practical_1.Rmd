...
In this practical, we will analyse data pertaining to customer waiting times in a governmental office. The data provided to us is the $waiting.csv$ file, which gives daily average waiting times from *January 3, 2017* to *October 26, 2018.*
...
- The daily plot chart below shows a continue oscillation of the data during the 447 observations.
- There are high deviation values regarding the mean in each day.
- The average per weekday are similar within them, nevertheless, the highest average is made on friday.
...
*Conclusion*: our main observations regarding the analysis are the following:

- The upper bound with a normal approximation is very close the mean of the waiting time (Upper bound= 460, Mean= 441).
- The probability that the upper bound will not be exceeded during the 1-year return period is quite high, because the upper bound do not consider the extreme values and it tends to concentrate in the middle due to the normal approximation.
- Thanks to the QQ-plot we can confirm that the data is not normal, as it is right-skewed (or positively skewed), in fact we can see deviations from the straight line in both extrems. 
...
- First, we determine the block size and the maximum value for each of them. For this exercise we choose weekly blocks.  

`Block Maxima:` $$M_{i}=max\{X_{(i−1)n+1},\cdots,X_{in}\},i=1,⋯,m$$

- Then, we fit the a Generalized Extreme Value, assuming that it follows a \(GEV(\mu_{n},\sigma_{n},\xi)\) distribution and we will focus on maximizing the log-likelihood.  
...
*Conclusions*: our main observations are the following:
...
We propose to use following steps:

- First, we need stablish a large enough threshold \(u\), with 
$$\mathbb{P}\left(X_{i}-u>y|X_{i}>u\right)$$. To be able to do it, we apply a mean residual plot and we select the threshold when the plot start to look linear.

- Then, we fit a Generaliaze Pareto distribution.  
...
We will use likelihood-ratio test for two nested extreme value distribution models, the function used comes from the `extRemes package`.
